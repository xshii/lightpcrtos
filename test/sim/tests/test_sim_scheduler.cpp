...
TEST_F(SimSchedulerTest, CreateTask) {
    TaskHandle handle;
    int ret = HAL_SCHEDULER_CreateTask(DummyTaskFunc, nullptr,
...
TEST_F(SimSchedulerTest, CreateMultipleTasks) {
    ...
    for (int i = 0; i < 5; i++) {
        int ret = HAL_SCHEDULER_CreateTask(DummyTaskFunc, nullptr,
                                           TASK_PRIORITY_NORMAL, &handles[i]);
        EXPECT_EQ(HAL_OK, ret);
...
TEST_F(SimSchedulerTest, DeleteTask) {
    TaskHandle handle;
    HAL_SCHEDULER_CreateTask(DummyTaskFunc, nullptr,
                            TASK_PRIORITY_NORMAL, &handle);
...
TEST_F(SimSchedulerTest, TaskPriorities) {
    ...
    HAL_SCHEDULER_CreateTask(DummyTaskFunc, nullptr,
                            TASK_PRIORITY_LOW, &lowPrioTask);
    HAL_SCHEDULER_CreateTask(DummyTaskFunc, nullptr,
                            TASK_PRIORITY_NORMAL, &normalPrioTask);
    HAL_SCHEDULER_CreateTask(DummyTaskFunc, nullptr,
                            TASK_PRIORITY_HIGH, &highPrioTask);
...
TEST_F(SimSchedulerTest, TaskWithArguments) {
    ...
    int ret = HAL_SCHEDULER_CreateTask(DummyTaskFunc, &taskArg,
                                       TASK_PRIORITY_NORMAL, &handle);
    EXPECT_EQ(HAL_OK, ret);
}
